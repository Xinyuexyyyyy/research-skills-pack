...
│ 命中类型      │ 调用                              │
├─────────────┼──────────────────────────────────┤
│ 学术综述      │ academic-deep-research（入口层）    │
...
判断方式:**关键词 + 任务结构 + 用户显式声明**,三者综合。

---

## 路由表

### 1. 命中 `academic-deep-research`(学术科研入口层)
...
综合类作为 fallback,常常需要**调起其他子包的能力**(查学术 + 查市场 + 查需求)。这部分协议在 `research-comprehensive` 模板包做时再定,本文件只规定路由本身。
